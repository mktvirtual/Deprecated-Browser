...
function deprecated(color){
	...
	H:el.clientHeight+1,w:this.imgSize[el.vmlBg].width,h:this.imgSize[el.vmlBg].height,L:el.offsetLeft,T:el.offsetTop,bLW:el.clientLeft,bTW:el.clientTop};fudge=size.L+size.bLW==1?1:0;makeVisible=function(vml,l,t,w,h,o){vml.coordsize=w+","+h;vml.coordorigin=o+","+o;vml.path="m0,0l"+w+",0l"+w+","+h+"l0,"+h+" xe";vml.style.width=w+"px";vml.style.height=h+"px";vml.style.left=l+"px";vml.style.top=t+"px"};makeVisible(el.vml.color.shape,size.L+(el.isImg?0:size.bLW),size.T+(el.isImg?0:size.bTW),size.W-1,size.H-
	1,0);makeVisible(el.vml.image.shape,size.L+size.bLW,size.T+size.bTW,size.W,size.H,1);bg={X:0,Y:0};if(el.isImg){bg.X=parseInt(thisStyle.paddingLeft,10)+1;bg.Y=parseInt(thisStyle.paddingTop,10)+1}else for(b in bg)if(bg.hasOwnProperty(b))this.figurePercentage(bg,size,b,thisStyle["backgroundPosition"+b]);el.vml.image.fill.position=bg.X/size.W+","+bg.Y/size.H;bgR=thisStyle.backgroundRepeat;dC={T:1,R:size.W+fudge,B:size.H,L:1+fudge};altC={X:{b1:"L",b2:"R",d:"W"},Y:{b1:"T",b2:"B",d:"H"}};if(bgR!="repeat"||
	el.isImg){c={T:bg.Y,R:bg.X+size.w,B:bg.Y+size.h,L:bg.X};if(bgR.search("repeat-")!=-1){v=bgR.split("repeat-")[1].toUpperCase();c[altC[v].b1]=1;c[altC[v].b2]=size[altC[v].d]}if(c.B>size.H)c.B=size.H;el.vml.image.shape.style.clip="rect("+c.T+"px "+(c.R+fudge)+"px "+c.B+"px "+(c.L+fudge)+"px)"}else el.vml.image.shape.style.clip="rect("+dC.T+"px "+dC.R+"px "+dC.B+"px "+dC.L+"px)"},figurePercentage:function(bg,size,axis,position){var horizontal,fraction;fraction=true;horizontal=axis=="X";switch(position){case "left":case "top":bg[axis]=
	0;break;case "center":bg[axis]=0.5;break;case "right":case "bottom":bg[axis]=1;break;default:if(position.search("%")!=-1)bg[axis]=parseInt(position,10)/100;else fraction=false}bg[axis]=Math.ceil(fraction?size[horizontal?"W":"H"]*bg[axis]-size[horizontal?"w":"h"]*bg[axis]:parseInt(position,10));if(bg[axis]%2===0)bg[axis]++;return bg[axis]},fixPng:function(el){el.style.behavior="none";var lib,els,nodeStr,v,e;if(el.nodeName=="BODY"||el.nodeName=="TD"||el.nodeName=="TR")return;el.isImg=false;if(el.nodeName==
	...
	if (document.createElement){ //Testando se é possível criar elementos pelo DOM
		...
		readMoore.innerHTML = 'Atualize-o gratuitamente por um dos navegadores ao lado. <a style="color:#000;" id="read_moore_link" href="#">Saiba mais</a>';
		leftSide.appendChild(info);	
		leftSide.appendChild(readMoore);
		
		document.getElementById("read_moore_link").addEventListener('click',aboutDeprecated,false)
